...
def create_list_for_video(args, name, video_list):
    ...
    for video in tqdm(video_list):
        ...
        path =  os.path.join('./data/Free-Music-Archive', video_path)
        meta_path = os.path.join(video_path, 'meta.json')
        with open(meta_path, "r") as f:
            meta_dict = json.load(f)
        if not meta_dict['ifstereo']:
            continue    
        ...
        if name == 'train':
            ...
            frame_nums = int(audio_length * 10) - 30
            if frame_nums <= 0:
                continue
            rand = np.random.choice(frame_nums, num, replace=False)
            for i in rand:
                sample = {
                    'path': path,
                    'start_time': i
